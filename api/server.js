...
io.on("connection", (socket) => {
  ...
  socket.on("soldier:join", async (data) => {
    const { callsign, name, janusFeed } = data;
    if (!callsign) return;

    try {
      const result = await pool.query(
        `INSERT INTO soldiers (callsign, name, janus_feed, is_online)
         VALUES ($1, $2, $3, true)
         ON CONFLICT (callsign) DO UPDATE
         SET is_online = true, janus_feed = COALESCE($3, soldiers.janus_feed),
             name = COALESCE($2, soldiers.name)
         RETURNING *`,
        [callsign, name || callsign, janusFeed || null]
      );
  ...
  // อัพเดท janus feed id (หลัง publish สำเร็จ)
  socket.on("soldier:feed", async (data) => {
    if (!socket.soldierId) return;
    await pool.query(
      `UPDATE soldiers SET janus_feed = $1 WHERE id = $2`,
      [data.feedId, socket.soldierId]
    );
    io.emit("soldier:feed_update", {
      soldierId: socket.soldierId,
      callsign: socket.callsign,
      feedId: data.feedId,
    });
